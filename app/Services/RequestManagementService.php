    ...
    private function paginateInitiativeTab(
        ...
        if ($initiative->form_route) {
            $query = match ($initiative->form_route) {
                ...
                default => CustomRequest::where('initiative_id', $initiative->id)->where(fn ($q) => $q->where('status', '!=', 'approved')->orWhere('updated_at', '>=', $cutoff))->with(['processedBy', 'initiative'])->latest(),
            };
        } else {
        ...
        if ($search) {
            if (in_array($initiative->form_route, ['forms.medicine.create', 'forms.silid.create'], true)) {
