...
class NewsManagementTest extends TestCase
{
    ...
    public function test_admin_can_perform_full_news_article_crud(): void
    {
        ...
        $responseEdit->assertSee('Incredible Basketball Championship Win');
    ...
    public function test_public_pages_load_fallback_news_when_database_is_empty(): void
    {
        ...
        // Assert we see fallback details matching the design layout requirements on landing
        $response->assertSee('Record-Breaking, Stunning Performance Leads The Swimmer');
        $response->assertSee('Romania and Bulgaria fully join Europe');

        // Assert we see trending fallback on the news page
        $responseNews = $this->get('/news');
        $responseNews->assertOk();
        $responseNews->assertSee('Innovative Farming Technology Transforms Local Agriculture Practices');
        
        // Assert fallback show page works
        $responseShow = $this->get('/news/record-breaking-swimmer-victory');
        $responseShow->assertOk();
        $responseShow->assertSee('Record-Breaking, Stunning Performance Leads The Swimmer');
    }
