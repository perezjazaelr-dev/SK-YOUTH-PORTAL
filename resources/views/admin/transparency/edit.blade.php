    ...
    <div class="flex-1 flex flex-col min-w-0">
        <div class="p-4 md:p-8 space-y-6 flex-1 overflow-y-auto max-w-3xl">
            ...
            <div class="pb-4 border-b border-slate-100">
                <h1 class="text-xl md:text-2xl font-black text-slate-800 font-display uppercase">Edit Transparency Post</h1>
                <p class="text-xs text-slate-500 mt-1">{{ $transparency->title }}</p>
            </div>
            <form method="POST" action="{{ route('admin.transparency.update', $transparency) }}" enctype="multipart/form-data" class="card space-y-5">
                @csrf @method('PUT')
                @include('admin.transparency._form', ['post' => $transparency, 'categories' => $categories])
