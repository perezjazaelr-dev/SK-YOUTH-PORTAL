...
<div x-data="{ mobileSidebar: false }" class="flex-1 flex flex-col md:flex-row bg-[#f8fafc]">
    ...
    <div class="flex-1 flex flex-col min-w-0">
        <header class="bg-white border-b border-slate-100 h-16 px-4 flex items-center justify-between md:hidden shrink-0">
            ...
            <span class="text-xs font-bold uppercase text-slate-800 font-display">Add Official</span>
            <div class="w-11"></div>
        </header>

        <div class="p-4 md:p-8 space-y-6 flex-1 overflow-y-auto max-w-3xl">
            <nav class="text-xs font-semibold uppercase tracking-wider pb-4 border-b border-slate-100 space-x-2">
                ...
                <span class="text-slate-300">/</span>
                <span class="text-slate-800">Add</span>
            </nav>

            <div class="pb-4 border-b border-slate-100">
                <h1 class="text-xl md:text-2xl font-black text-slate-800 font-display uppercase">Add Official Profile</h1>
                <p class="text-xs text-slate-500 mt-1">Publish an elected SK officer profile to the public officials directory.</p>
            </div>

            <form method="POST" action="{{ route('admin.officials.store') }}" enctype="multipart/form-data" class="card space-y-5">
                @csrf
                ...
                <div class="pt-4 border-t border-slate-100 flex flex-col-reverse sm:flex-row sm:justify-end gap-2">
                    ...
                    <button type="submit" class="btn-primary text-xs min-h-11">Publish Profile</button>
                </div>
            </form>
        </div>
    </div>
