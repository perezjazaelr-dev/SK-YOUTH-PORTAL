...
@section('content')
<div x-data="{ mobileSidebar: false, selectedLog: null }" class="flex-1 flex flex-col md:flex-row bg-[#f8fafc]">
                ...
                <form id="filterForm" method="GET" action="{{ route('admin.logs.index') }}" class="space-y-4">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-12 gap-4">
                        ...
                        <div class="md:col-span-3 relative">
                            ...
                            <div class="absolute inset-y-0 right-0 pr-3.5 flex items-center pointer-events-none text-slate-450">
                        ...
                        <div class="md:col-span-3 relative">
                            ...
                            <div class="absolute inset-y-0 right-0 pr-3.5 flex items-center pointer-events-none text-slate-455">
                                <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 9l-7 7-7-7"></path></svg>
                            </div>
                        </div>
                    </div>
                        ...
                        <div class="flex items-center gap-3">
                            ...
                            <div class="relative w-32 shrink-0">
                                <select 
                                    name="limit" 
                                    onchange="this.form.submit()"
                                    class="block w-full py-2 pl-3 pr-8 bg-slate-50/70 border border-slate-200/60 rounded-2xl text-[11px] text-slate-655 outline-none focus:bg-white focus:border-[#1e40af] focus:ring-4 focus:ring-blue-600/5 transition cursor-pointer appearance-none font-semibold"
                                >
                                ...
                                <div class="absolute inset-y-0 right-0 pr-2.5 flex items-center pointer-events-none text-slate-450">
            ...
            <div class="card p-0 overflow-hidden bg-white border border-slate-100">
                @if($logs->isEmpty())
                    <div class="text-center py-16 px-4 space-y-4">
                        ...
                        <div>
                            <h3 class="text-sm font-bold text-slate-700 uppercase tracking-wider">No Activity Logged</h3>
                            <p class="text-xs text-slate-400 mt-1 max-w-sm mx-auto">Either no actions match your filter criteria or no system events have occurred yet.</p>
                        </div>
                    </div>
                @else
                        ...
                        <table class="w-full text-left border-collapse">
                            <thead>
                                <tr class="bg-slate-50 border-b border-slate-100 text-[10px] font-bold text-slate-400 uppercase tracking-wider font-display">
                                    ...
                                    <th class="p-4">Subject Target</th>
                                ...
                                @foreach($logs as $log)
                                    @php
                                        ...
                                        $badgeColor = match(true) {
                                            str_contains($log->action, 'created') || str_contains($log->action, 'registered') => 'bg-emerald-50 text-emerald-700 border-emerald-150',
                                            str_contains($log->action, 'updated') || $log->action === 'status_changed' => 'bg-amber-50 text-amber-700 border-amber-150',
                                            str_contains($log->action, 'deleted') || str_contains($log->action, 'cancelled') || str_contains($log->action, 'failed') => 'bg-rose-50 text-rose-700 border-rose-150',
                                            $log->action === 'pii_accessed' => 'bg-indigo-50 text-indigo-700 border-indigo-150',
                                    ...
                                    <tr class="hover:bg-slate-50/50 transition duration-150">
                                        <!-- Timestamp -->
                                        <td class="p-4 pl-6 text-slate-500 font-mono">
                                        ...
                                        <td class="p-4">
                                            @if($log->user)
                                                ...
                                                <span class="text-[10px] text-slate-400 block font-mono">{{ $log->user->email }}</span>
                                            @else
                                        ...
                                        <td class="p-4 pr-6 text-right">
                                            <button @click='selectedLog = {
                                                    ...
                                                    class="inline-flex items-center px-2.5 py-1 border border-slate-200 text-slate-600 hover:text-[#1e40af] hover:border-[#1e40af] font-bold rounded-lg transition text-[10px] uppercase tracking-wider active:scale-95">
                                                Inspect
                                            </button>
                                        </td>
                                    </tr>
    ...
    <div x-show="selectedLog !== null" 
        ...
        <div class="bg-white rounded-3xl overflow-hidden shadow-2xl transform transition-all sm:w-full sm:max-w-lg mx-auto z-10 border border-slate-100 max-h-[90vh] flex flex-col"
            ...
            <div class="bg-gradient-to-r from-blue-700 to-blue-900 px-6 py-4 flex items-center justify-between text-white shrink-0">
                <div>
                    <span class="text-[9px] font-black uppercase tracking-widest text-blue-200">Log Inspector</span>
                    <h3 class="text-sm font-extrabold uppercase tracking-wide font-display mt-0.5">Audit Entry Details</h3>
                </div>
            ...
            <div class="p-6 space-y-4 overflow-y-auto flex-1 text-xs">
                ...
                <div class="grid grid-cols-2 gap-4 bg-slate-50 p-4 rounded-2xl border border-slate-100">
                    <div>
                        <span class="text-[9px] font-black uppercase tracking-wider text-slate-400 block font-display">Event Action</span>
                        <span class="font-extrabold text-[#1e40af] text-[11px] uppercase tracking-wide" x-text="selectedLog ? selectedLog.action.replace('_', ' ') : ''"></span>
                    </div>
                    <div>
                        <span class="text-[9px] font-black uppercase tracking-wider text-slate-400 block font-display">Timestamp</span>
                        <span class="font-mono text-slate-700 text-[10px]" x-text="selectedLog ? selectedLog.date : ''"></span>
                    </div>
                    <div>
                        <span class="text-[9px] font-black uppercase tracking-wider text-slate-400 block font-display">Actor / Performed By</span>
                        <span class="font-bold text-slate-800 text-[11px]" x-text="selectedLog ? selectedLog.actor : ''"></span>
                        <span class="text-[9px] text-slate-400 block font-mono" x-text="selectedLog && selectedLog.actor_email ? selectedLog.actor_email : ''"></span>
                    </div>
                    <div>
                        <span class="text-[9px] font-black uppercase tracking-wider text-slate-400 block font-display">IP Address & Subject</span>
                        <span class="font-mono text-slate-600 block text-[10px]" x-text="selectedLog ? 'IP: ' + selectedLog.ip : ''"></span>
                        <span class="font-mono text-[#1e40af] text-[10px] block mt-0.5" x-text="selectedLog ? 'Subject: ' + selectedLog.subject : ''"></span>
                    </div>
                </div>

                <!-- Payload Inspection Block -->
                <div class="space-y-2">
                    <span class="text-[9px] font-black uppercase tracking-wider text-slate-400 block font-display">Payload Data & Changes</span>
            ...
            <!-- Modal Footer -->
            <div class="bg-slate-50 px-6 py-4 flex items-center justify-end border-t border-slate-100 shrink-0">
                <button @click="selectedLog = null" class="px-4 py-2 bg-[#1e40af] text-white hover:bg-blue-700 font-bold rounded-xl text-[10px] uppercase tracking-wider transition active:scale-95">
                    Done
                </button>
            </div>
        </div>
