<section x-data="{
    ...
    <div class="relative h-[220px] sm:h-[360px] md:h-[450px] rounded-2xl sm:rounded-3xl overflow-hidden shadow-lg border border-slate-100 bg-slate-900 carousel-shadow">
        ...
        <template x-for="(slide, index) in slides" :key="index">
            <div x-show="activeSlide === index"
                 ...
                 class="absolute inset-0 bg-cover bg-center flex flex-col justify-end text-white p-4 sm:p-8 md:p-16"
                ...
                <div class="relative z-10 max-w-2xl space-y-3 sm:space-y-4">
                    ...
                    <div class="flex flex-col sm:flex-row flex-wrap gap-2 sm:gap-3 pt-1 sm:pt-2">
                        <a :href="slide.url1" @click.prevent="handleCtaClick(slide.url1)" class="btn-primary w-full sm:w-auto justify-center" x-text="slide.cta1"></a>
                        <a href="{{ route('track.index') }}" class="btn-outline text-white hover:text-[#1e40af] border-white/20 hover:bg-white w-full sm:w-auto justify-center">
                            Track Request
                        </a>
                    </div>
                </div>
        ...
        <button @click="prev()" class="absolute left-2 sm:left-4 top-1/2 -translate-y-1/2 w-8 h-8 sm:w-10 sm:h-10 rounded-xl bg-white/10 hover:bg-white/25 border border-white/10 backdrop-blur-md text-white flex items-center justify-center transition active:scale-95 group">
        ...
        <button @click="next()" class="absolute right-2 sm:right-4 top-1/2 -translate-y-1/2 w-8 h-8 sm:w-10 sm:h-10 rounded-xl bg-white/10 hover:bg-white/25 border border-white/10 backdrop-blur-md text-white flex items-center justify-center transition active:scale-95 group">
